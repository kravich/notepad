...
QRegularExpression FileSearcher::createRegexFromConfig(const SearchConfig &config)
{
    ...
    const QFlags<QRegularExpression::PatternOption> options = config.matchCase ? QRegularExpression::MultilineOption : QRegularExpression::MultilineOption | QRegularExpression::CaseInsensitiveOption;

    const QString regexString = config.matchWord ? "\\b" + config.searchString + "\\b" : config.searchString;
...
DocResult FileSearcher::searchPlainText(const SearchConfig &config, const QString &content)
{
    ...
    const QString searchString = (config.searchMode == SearchConfig::ModePlainTextSpecialChars) ? SearchString::unescape(config.searchString) : config.searchString;
...
void FileSearcher::run()
{
    ...
    const QFlags<QDirIterator::IteratorFlag> dirIteratorOptions = m_searchConfig.includeSubdirs ? (QDirIterator::Subdirectories | QDirIterator::FollowSymlinks) : QDirIterator::NoIteratorFlags;
