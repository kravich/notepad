...
QList<QSharedPointer<Editor>> DocEngine::loadDocumentsWithFeedback(const DocEngine::DocumentLoader &docLoader)
{
    ...
    {
        // Run Qt event loop to make UI more responsive
        // FIXME: Decide if this actually needed
...
bool DocEngine::writeFromString(QIODevice *io, const DecodedText &write)
{
    ...
    if (write.bom)
    {
        ...
        if (write.codec->mibEnum() == MIB_UTF_8)
        { // UTF-8
    ...
    if (outFileName.isLocalFile())
    {
        ...
        do {
            ...
            else
            {
                ...
                auto retryRoot = sudoProgram.isEmpty() ? nullptr : msgBox.addButton(tr("Retry as Root"), QMessageBox::AcceptRole);
